...
function LoginPage() {
  ...
  const handleSubmit = (event) => {
    ...
    Promise.all([
      fetch(`http://localhost:3000/db.php?table=Doctor&email=${email}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
        },
      }),
      fetch(`http://localhost:3000/db.php?table=Patient&email=${email}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
        },
      }),
      fetch(`http://localhost:3000/db.php?table=Admin&email=${email}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      .then(([doctorData, patientData, adminData]) => {
        const userFound =
          (doctorData &&
            doctorData.DocPassword &&
            password === doctorData.DocPassword) ||
          (patientData &&
